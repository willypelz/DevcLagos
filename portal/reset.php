...
if (Input::exists()) {
 if(Token::check(Input::get('token'))){
	  ...
	  if (!$validation->fails()) {
		...
		$query = DB::getInstance()->get("client", "*", ["tokencode" => Input::get('code')]);
		if ($query->count() === 1) {
         	foreach($query->results() as $row) {
			...
			if (Hash::make(Input::get('password'), $row->salt) === $row->password) {
			  ...
			  $Update = DB::getInstance()->update('client',[
			...
			$query = DB::getInstance()->get("freelancer", "*", ["tokencode" => Input::get('code')]);
			if ($query->count() === 1) {
	         	foreach($query->results() as $row) {
...
?>
	...
	<header id="header" headroom="" role="banner" tolerance="5" offset="700" class="navbar navbar-fixed-top navbar--white ng-isolate-scope headroom headroom--top">
	  <nav role="navigation">
		...
		<?php } else { ?>		 		        
	        <li class="header-nav__navigation-item <?php echo $active = ($basename == 'login') ? ' active' : ''; echo $active = ($basename == 'forgot') ? ' active' : ''; echo $active = ($editname == 'reset.php?id='. Input::get('id').'&code='. Input::get('code').'') ? ' active' : ''; ?>">
	  ...
	  <div class="container">
	   ...
	   <div class="row">
        ...
        <?php if (isset($error)) {
	        ...
	        <div class="alert alert-danger fade in">
	         <a href="#" class="close" data-dismiss="alert">&times;</a>
	         <strong><?php echo $lang['hasError']; ?></strong><?php echo $lang['reset_error']; ?>
		    </div>
	        <?php } ?>
	        ...
	        <div class="alert alert-danger fade in">
	         <a href="#" class="close" data-dismiss="alert">&times;</a>
	         <strong><?php echo $lang['hasError']; ?></strong> <?php echo $lang['notoken_error']; ?>
		    </div>
	        <?php } ?>
		   ...
		   <a href="#" class="close" data-dismiss="alert">&times;</a>
		   <strong><?php echo $lang['noError']; ?></strong> <?php echo $lang['reset_success']; ?></strong>.
		   </div>
		  <?php } ?>				
		   ...
		   <form method="post">
			...
			<div class="form-foot">
			 <div class="form-actions">					
			  ...
			  <input value="<?php echo $lang['submit']; ?>" class="form-btn" type="submit">
			 </div><!-- /.form-actions -->
			</div><!-- /.form-foot -->
		   </form>
